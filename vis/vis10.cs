        ...
        public string part2() {
            ...
            renderer.loop(cnt => {               
                ...
                if (cycle < cpu.spl.Count) {
                    ...
                    DrawCube(new Vector3(sx[0], 0, sy), 1, 0.1f, 1, BLUE);
                    DrawCube(new Vector3(sx[1], 0, sy), 1, 0.1f, 1, BLUE);
                    DrawCube(new Vector3(sx[2], 0, sy), 1, 0.1f, 1, BLUE);
                    DrawCube(new Vector3(rx, rz, py), 1, 1, 1, RED);
                    DrawCubeWires(new Vector3(rx, rz, py), 1, 1, 1, BLACK);
                }
                for (int d = 0; d <= cycle && d < cpu.disp.Count; d++) {
                    int dx = d % 40;
                    int dy = 1 + d / 40; 
                    if (cpu.disp[d] == '#') { 
                        DrawCube(new Vector3(dx, 0, dy), 0.8f, 0.2f, 0.8f, GREEN);
                        DrawCubeWires(new Vector3(dx, 0, dy), 0.8f, 0.2f, 0.8f, BROWN);
                    }
                }
                EndMode3D();
                return cnt > 2500;
